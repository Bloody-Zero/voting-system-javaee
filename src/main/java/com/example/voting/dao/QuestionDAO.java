...
public class QuestionDAO {
    ...
    /**
     * Находит вопрос с инициализированными коллекциями choices и vote.
     */
    public Question findByIdWithChoices(Long id) {
        EntityManager em = JPAUtil.getEntityManager();
        try {
            return em.createQuery(
                    "SELECT q FROM Question q LEFT JOIN FETCH q.choices LEFT JOIN FETCH q.vote WHERE q.id = :id",
                    Question.class)
                    .setParameter("id", id)
                    .getSingleResult();

...
public class QuestionService {
    ...
    /**
     * Возвращает вопрос с инициализированными коллекциями (choices, vote).
     */
    public Question getQuestionWithChoices(Long id) {
        if (id == null) {
            throw new IllegalArgumentException("ID вопроса не может быть null");
        }
        Question question = questionDAO.findByIdWithChoices(id);
        if (question == null) {
            throw new IllegalArgumentException("Вопрос с ID " + id + " не найден");
        }
        return question;
